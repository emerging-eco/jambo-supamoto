...
import { CHAIN_ID, CHAIN_NETWORK, CHAIN_RPC_URL, DefaultChainNetwork } from '@constants/chains';
...
export const ChainProvider = ({ children }: HTMLAttributes<HTMLDivElement>) => {
  ...
  async function initQueryClient() {
    try {
      if (!CHAIN_RPC_URL) throw new Error('CHAIN_RPC_URL is not set');
      const queryClient = await createQueryClient(CHAIN_RPC_URL as string);
  ...
  return (
    <ChainContext.Provider value={value}>
      {children}
      <Banner label='TEST' display={CHAIN_NETWORK !== 'mainnet'}></Banner>
